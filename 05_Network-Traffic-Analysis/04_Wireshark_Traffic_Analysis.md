...
![Open TCP port example](wireshark_opentcpport.png)
![Closed TCP port example](wireshark_closedtcpport.png)
...
![Open TCP SYN scan](wireshark_opensynscan.png)
![Closed port TCP SYN scan](wireshark_closedsynscan.png)
...
![Open and closed UDP ports (port 69 closed, port 68 open)](wireshark_opencloseudp.png)
...
![TCP Connect scan filter results](wireshark_totaltcpscan.png)
...
![UDP closed port filter results](wireshark_udpclosedports.png)
...
![UDP port range filter](wireshark_udpport68.png)
...
![Normal ARP request](wireshark_normalarprequest.png)
![Normal ARP reply](wireshark_arpreply.png)
![Conflicting ARP response - duplicate IP warning](wireshark_conflictingarp.png)
...
![ARP requests from attacker IP](wireshark_arpip.png)
...
![HTTP filter by attacker IP](wireshark_httpspecificip.png)
...
![POST request filter showing 6 entries](wireshark_6passwords.png)
...
![Client986 POST packet](wireshark_client986.png)
...
![Client354 POST packet](wireshark_nicework.png)
*Still on POST requests*
...
![DHCP hostname filter for Galaxy A30](wireshark_galaxya30.png)
...
![NBNS filter for LIVALJM](wireshark_livaljm.png)
...
![DHCP requested IP filter](wireshark_galaxya12.png)
...
![Kerberos CNameString filter for u5](wireshark_kerberosu5.png)
...
![Kerberos hostname filter using $](wireshark_kerberosname.png)
...
![SSH inside ICMP packets](wireshark_sshicmp.png)
...
![DNS data exfiltration domain](wireshark_dnsdataexfil.png)
...
![FTP 530 response code filter](wireshark_ftp530.png)
...
![FTP 213 file status response](wireshark_ftpfilesize.png)
...
![FTP uploaded document](wireshark_uploadeddoc.png)
...
![CHMOD command in FTP traffic](wireshark_chmod.png)
...
![User agent analysis](wireshark_useragents.png)
...
![Mozlila misspelling in user agent](wireshark_mozlila.png)
...
![Log4j jndi/Exploit filter results](wireshark_jndiexploit.png)
...
![Packet 444 User-Agent payload](wireshark_packet444.png)
*Used the starting attack packet and copied the entire User-Agent line and decoded with CyberChef*

![CyberChef base64 decode result](wireshark_cyberchef444.png)
...
![Wireshark Credentials window](wireshark_credentials.png)

- **Answer: 237**

**2. What is the packet number where "empty password" was submitted?**

![Empty password packet](wireshark_emptypass.png)
...
![IPFirewall deny IP rule](wireshark_denyip.png)
