...
![THM screenshot explaining header artifacts and body analysis](email_header_artifacts.png)
...
1. Use Talos Reputation Center to look up `malware-test.com`. What is the web reputation assigned to this domain?
   ...
   ![Reputation lookup for malware-test.com](malwaretest_com.png)
...
![VirusTotal example](virustotal_example.png)
...
![PhishTool example](phishtool_example.png)

![PhishTool analysis tabs](phishtool_example2.png)

![Resolving the case with PhishTool](phishtool_resolvescase.png)
...
![Phishing scenario overview](task7_phishing_scenario.png)

### Task Questions

1. What reputable brand is this email tailored to impersonate?

   ![Phishing email analysis](spoofed_netflixemail.png)
   - **Answer: Netflix**
...
3. In Thunderbird mail use `View` > `Message Source`. What is the `Received: from` IP address?
   ...
   ![Find the IP under message source](netflix_email_source.png)
...
![Text report shortcut in ANY.RUN](getreport_anyrun.png)

### Task Questions

1. How does ANYRUN classify this suspected phishing email?

   ![Suspicious activity classification in ANY.RUN](anyrun_netflixemail.png)
...
4. Check out the ANYRUN text report on the phishing email. Which IP address associated with the process `AcroRd32.exe` is flagged as malicious?
   ...
   ![Malicious IP for Netflix email](netflixemail_connections.png)
   - **Answer: 2.16.107.24**

5. Continue investigating the text report. Which Windows process is classed as `Potentially Bad Traffic`?

   ![Malicious Windows process for Netflix email](netflixemail_threat.png)
   - **Answer: svchost.exe**
...
![Preview of Task 9 THM screenshot](excel_task9preview.png)

![Text report of malicious file](task9_anyrun_secondexample.png)
...
4. Check out the ANYRUN text report. What IP address is associated with the malicious domain `biz9holdings.com`?
   ...
   ![Network activity text report](anyrun_biz9holdings.png)
...
6. What vulnerability does this malicious attachment attempt to exploit?
   ...
   ![CVE answer in ANY.RUN report](phshing_emailcve.png)
