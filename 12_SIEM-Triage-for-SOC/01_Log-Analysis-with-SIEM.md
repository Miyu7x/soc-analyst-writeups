...
Network-based logs keep a record of anything that touches the web/network: firewalls, routers, IDS and IPS, and others.
  - Monitors traffic and connections across your network.
...
Organizations have their own web app most of the time, and this is where web-based logs come from.
  - Attackers often gain access to an organization through web app vulnerabilities, so it's important for SOCs to pay close attention to their web events.
...
However, this task will touch upon two other important topics: time pitfalls and normalization in SIEM.
...
Let's say you're working in UTC-2, but the logs in Splunk SIEM are normalized to UTC+2. If it's 5 PM for you, those same logs might appear as 9 PM in Splunk. That doesn't mean the logs were ingested four hours late; it's just a difference in time settings.
...
WinEventLogs, or Windows Event Logs, include hundreds of unique log files beyond the common three we're used to: Security, System, and Application logs.
...
In that task, we examined a number of important log sources for a SOC L1 analyst and the value they can provide. Of course, there are many other logs that we will explore together in future rooms.
...
For our SOC investigation, our alert informed us of a suspicious network connection on port 5678 on host WIN-105. To read the results in Splunk, we set table parameters for what we want to see and filter by what we were given — source port and host name — then, on the next line, add the table parameters: time for the record and image, to see which process established the network connection.

Filter:
```spl
index=Task4 ComputerName=WIN-105 DestinationPort=5678
| table _time Image SourceIp SourcePort DestinationIp DestinationPort Protocol
```
...
Since we found the malicious process which spawned the connection, we'll build our search around those events using EventType and EventCode. Let's expand our view with the CommandLine and ParentCommandLine fields. **Note:** remember to add the **Message** parameter to the table as well, so we get further information on each event — this is crucial for readability.
...
As we investigate the alerts and follow the timeline, observing the CommandLine field, we can see a **schtasks** command that was run to install on logon: **Office365 Install**.
...
When investigating Linux logs in a SIEM, our go-to source is **auth.log**, which provides authentication activity such as:
  - User logins
  - Sudo usage
...
Logs involving system-level events are monitored in **syslog**:
  ...
  - Helpful for building timelines and understanding system behavior
...
We detected three interesting events. First, we can see that a suspicious pnr5433sw.sh file from the /tmp folder is being executed via cron every 5 minutes. Next, there are clear signs of a Perl reverse shell attempting to establish a connection to 10.10.101.12 on port 9999.

<p align="center">
<img src=screenshots/analysis_syslog.png width="700">
</p>

As mentioned earlier, in addition to the core data sources, it's very common to see tools like auditd and osquery used in real-world environments. These tools fall outside the scope of this room, so if you're interested in learning more about them, feel free to explore other related rooms available on the platform.
...
Splunk makes it easy for SOCs to search logs for specific keywords — crucial for finding **newly created** connections, accounts, users, and more. We can filter using these parameters.
...
As a SOC, we should be aware that threat actors will always try to escalate privileges or poke around in sudo or su.
...
Since we now know the user that has been compromised, let's take a look at all the events run by the user **jack-brown**. We can observe the brute-force attempts from the malicious IP on port 54446.
...
In our previous searches we were able to see failed login attempts for jack-brown, but let's refine it to find those specifically.
...
Persistence comes in many different forms. When searching for these, it's **important** to do a thorough search across all script types, such as: ("python" OR "perl" OR "ruby" OR ".sh" OR "bash" OR "nc").

For this room's purposes, a simple search for cron jobs will suffice.
...
Most companies have a website, which means they also have a web server running it.
  - Common web servers: Apache, Nginx, and others.
  - **Access Logs** provide us with web requests and resources:
      - Scanning
      - DDoS attempts
      - Web-based attacks
      - Web shells
...
Imagine you are a SOC L1 analyst on shift, and you've just received an alert about a possible brute-force attack targeting a WordPress login page. To investigate, we recommend starting by identifying the login page URL (e.g., /wp-login.php) and filtering for POST requests. Brute-force activity involves many repeated attempts, so set a threshold like count > 25 over a 5-minute span. Group the results by clientip to pinpoint the source of the activity. Other fields, such as user-agent, can also provide useful context. Below is the ready query.
...
Your shift is almost over, but don't rush to close your laptop just yet — a new alert has just come in, likely the last one for today, about a possible DDoS attack. When checking for signs of a potential DDoS in Splunk, look for status code 503, which can mean the server is overloaded. Review for huge request counts in a short time, for example, more than 100,000 in 10 minutes, filtered by IP. Review the targeted domain, which user-agent was used, and the URI path to spot patterns or attacker fingerprints. Below is the ready query.
...
As a SOC analyst, a high number of requests in a short amount of time should be lighting up alerts in your head — this could signal a brute-force attempt. In this case, we're searching for over 25 requests within a span of 5 minutes.
...
In our first question, we observed the UserAgent the attacker was using: [WPScan](https://wpscan.com/wordpress-security-scanner), a WordPress security scanner.
