...
![The ELK security tools](T2_elk_elasticsearch_stack.png)
...
![All components of the Beats family](T2_beats_family.png)
...
![Numbered elements of the Discover Dashboard](T4_numbered_discover_dashboard.png)
...
1. Select the index vpn_connections and filter from 31st December 2021 to 2nd Feb 2022. How many hits are returned?
   ![Filtered vpn_connections Dec 31 21 to Feb 2 22](T4_q1_firstelastic_search.png)
   - **Answer: 2861**

2. Which IP address has the maximum number of connections?
   ![Maximum number of IP connections](T4_q2_highestip_connections.png)
   - **Answer: 238.163.231.224**

3. Which user is responsible for the overall maximum traffic?
   ![James had the most traffic](T4_q3_james_hightraffic.png)
   - **Answer: James**

4. Apply Filter on UserName Emanda; which SourceIP has max hits?
   ![Filtered by Emanda to find max SourceIP](T4_q4_emanda_traffic.png)
   - **Answer: 107.14.1.247**

5. On 11th Jan, which IP caused the spike observed in the time chart?
   ![Which IP caused a spike on Jan 11](T4_q5_ipspike_jan11.png)
...
7. Create a table with the fields IP, UserName, Source_Country and save.
   ...
   ![Selected fields for the table](T4_q7_selected_filters.png)
...
![Search using KQL](T5_kql_search_language.png)
...
![Free text search](T5_free_textsearch.png)
![No results for partial term searched](T5_noresult_forpartial_words.png)
...
![KQL United wildcard search](T5_kql_united.png)
...
1. Create a search query to filter the logs where Source_Country is the United States and show logs from User James or Albert. How many records were returned?
   ![Filter by United States and James or Albert](T5_q1_james_albert.png)
   - **Answer: 161**
...
2. A user Johny Brown was terminated on the 1st of January, 2022. Create a search query to determine how many times a VPN connection was observed after his termination.
   ![Search Username Johny Brown](t5_q2_johnybrown.png)
   - **Answer: 1**
...
![Visualize directly from field filters](T6_visualize_frommenu.png)

### Task Questions

1. Which user was observed with the greatest number of failed attempts?
   ![T6 Filter by failed action](T6_simon_failedlogins.png)
   - **Answer: Simon**
...
![Create new dashboard](T7_create_newdashboard.png)
